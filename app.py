...
with st.sidebar:
    ...
    st.success("✅ ISA S75.01 / IEC 60534-2-1")
    st.success("✅ ISA RP75.23 (Sigma Method)")
    st.success("✅ IEC 60534-8-3 (Noise)")
    st.success("✅ API 6D")
...
if st.session_state.step == 1:
    ...
    st.subheader("Safety Factor Recommendation")
...
# --- STEP 3: Enhanced Sizing Calculations ---
        ...
        if st.session_state.input_data['fluid_type'] == 'Gas/Vapor' and 'flow_regime' in results:
            with st.expander("Gas Flow Analysis", expanded=True):
                ...
                if results.get('choking_warning'):
                    st.warning(results['choking_warning'])
                if results.get('velocity_warning'):
                    st.warning(results['velocity_warning'])
    ...
    with col2:
        st.button("Continue to Enhanced Noise Analysis ➡️", on_click=next_step)
...
# --- STEP 4: Enhanced Noise Prediction ---
    ...
    try:
        ...
        with col2:
            ...
            if noise_level > 110:
                ...
                st.warning("🔊 HIGH - Mitigation recommended")
            else:
        ...
        if noise_results.get('warning'):
            st.warning(noise_results['warning'])
        ...
        if noise_control_recs:
            st.write("**Noise Control Recommendations:**")
    ...
    with col2:
        st.button("Continue to Enhanced Actuator & Materials ➡️", on_click=next_step)
...
# --- STEP 5: Enhanced Actuator, Materials & Final Selection ---
    ...
    with st.expander("Enhanced Actuator Sizing", expanded=True):
        ...
        with col2:
            st.metric("Safety Factor Applied", f"{actuator_results['safety_factor_used']:.1f}")
    ...
    with st.expander("Enhanced Valve Dynamic Characteristic", expanded=True):
        try:
            ...
            st.warning(f"Could not generate characteristic plot: {e}")
...
# --- STEP 6: Enhanced Summary and Report Generation ---
    ...
    with col2:
        ...
        if validation.get('overall_valid', False):
            ...
            st.metric("Sizing Status", "⚠️ REVIEW NEEDED")
